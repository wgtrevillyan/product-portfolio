...
6. **Product pages** — Generates `product_pages/<slug>.html` for each product with correct Open Graph and Twitter Card meta (title, description, image) so LinkedIn and other crawlers show the right thumbnail when sharing product links. Converts WebP thumbnails to PNG in `images/products/og/` for og:image (LinkedIn supports only PNG/JPEG/GIF). Updates `vercel.json` with per-product rewrites so `/products/:slug` serves the generated file.
...
│   └── products/og/      # Generated by build; PNG versions of thumbs for LinkedIn (og:image)
├── js/
